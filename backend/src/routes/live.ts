...
export async function liveCallRoutes(app: FastifyInstance) {
  ...
  app.get('/stream', { websocket: true }, (connection, req) => {
    ...
    connection.socket.on('message', async (message: string) => {
      try {
        ...
        if (fullTranscript.trim().length < 5) return;

        const currentTime = Date.now();
        // Throttle to 5 seconds
        if (currentTime - lastGeminiCallTime < 5000) return;
...
`;
        ...
        const response = await ai.models.generateContent({
          model: 'gemini-3.6-flash', // Updated to 3.6 flash
          contents: prompt,
          config: {
