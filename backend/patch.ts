...
content = content.replace(
  /const response = await ai\.models\.generateContent\(\{[\s\S]*?model: 'gemini-[^']*',[\s\S]*?contents: prompt,[\s\S]*?config: \{[\s\S]*?responseMimeType: 'application\/json',[\s\S]*?temperature: 0.1,[\s\S]*?\}[\s\S]*?\}\);[\s\S]*?const content = response\.text;[\s\S]*?if \(!content\) throw new Error\('No response from Gemini'\);[\s\S]*?const parsed = JSON\.parse\(content\);/m,
...
content = content.replace(
  /const response = await ai\.models\.generateContent\(\{[\s\S]*?model: 'gemini-[^']*',[\s\S]*?contents: prompt,[\s\S]*?config: \{[\s\S]*?responseMimeType: 'application\/json',[\s\S]*?temperature: 0.2,[\s\S]*?\}[\s\S]*?\}\);[\s\S]*?const content = response\.text;[\s\S]*?if \(!content\) throw new Error\('No response from Gemini'\);[\s\S]*?const parsed = JSON\.parse\(content\);/m,
  `const parsed = await generateWithRetry('gemini-3.5-flash', prompt, { path: filePath, mimeType });`
);
...
content = content.replace(
  /const response = await ai\.models\.generateContent\(\{[\s\S]*?model: 'gemini-[^']*',[\s\S]*?contents: prompt,[\s\S]*?config: \{[\s\S]*?responseMimeType: 'application\/json',[\s\S]*?temperature: 0.3,[\s\S]*?\}[\s\S]*?\}\);[\s\S]*?const content = response\.text;[\s\S]*?if \(!content\) throw new Error\('No response from Gemini'\);[\s\S]*?const parsed = JSON\.parse\(content\);/m,
  `const parsed = await generateWithRetry('gemini-3.5-flash', prompt);`
);
...
// synthesizeInterview
content = content.replace(
  /const response = await ai\.models\.generateContent\(\{[\s\S]*?model: 'gemini-[^']*',[\s\S]*?contents: prompt,[\s\S]*?config: \{[\s\S]*?responseMimeType: 'application\/json',[\s\S]*?temperature: 0.3,[\s\S]*?\}[\s\S]*?\}\);[\s\S]*?const synthesisContent = response\.text;[\s\S]*?if \(!synthesisContent\) throw new Error\('No response from Gemini'\);[\s\S]*?return JSON\.parse\(synthesisContent\);/m,
  `const parsed = await generateWithRetry('gemini-3.5-flash', prompt);\n  return parsed;`
);

// generateEvaluationReport
content = content.replace(
  /const response = await ai\.models\.generateContent\(\{[\s\S]*?model: 'gemini-[^']*',[\s\S]*?contents: prompt,[\s\S]*?config: \{[\s\S]*?responseMimeType: 'application\/json',[\s\S]*?temperature: 0.2,[\s\S]*?\}[\s\S]*?\}\);[\s\S]*?const evalContent = response\.text;[\s\S]*?if \(!evalContent\) throw new Error\('No response from Gemini'\);[\s\S]*?return JSON\.parse\(evalContent\);/m,
  `const parsed = await generateWithRetry('gemini-3.5-flash', prompt);\n  return parsed;`
);

// Also need to add back my empty email check that was lost in the revert!
content = content.replace(
