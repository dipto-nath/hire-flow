...
export async function interviewRoutes(app) {
    ...
    app.get('/', async (request) => {
        const { candidateId, jobId, status, limit = 50, offset = 0 } = request.query;
        const where = {};
        if (candidateId)
    ...
    app.patch('/:id', async (request, reply) => {
        ...
        if (updateData.status) {
            ...
            await prisma.candidate.update({
                where: { id: interview.candidateId },
                data: { interviewStatus: statusMap[updateData.status] },
            });
        }
        return interview;
    });
    ...
    app.post('/generate-prep', async (request, reply) => {
        const { candidateId, jobId, numQuestions } = request.body;
